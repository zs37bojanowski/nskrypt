...
\begin{enumerate}
\item{\begin{verbatim}
W pewnym folderze znajdują się między innymi pliki: czter4, pli7k, plik, plik1k.
Napisz takie polecenie, które wylistuje dokładnie te pliki.
Odp: ls {p,c}*{4,k}
\end{verbatim}}
\item{\begin{verbatim}
Ala, Klara i Laura są sekretarkami i mają konta na jednym serwerze. Jesteś adminem.
Napisz jedno polecenie, które wyśle do ich ~ plik "telefony&adresy".
Odp: cp ./telefony&adresy /home/*{L,l}a*
\end{verbatim}}
\item{\begin{verbatim}
Księgowy chce skopiować do swojego ~ bilanse z lat 2011,2013,2014 lecz nie z lat
pozostałych z zakresu 2000 do 2015. Bilanse przechowuje w katalogu lata 2000_20.
Pomóż mu napisać polecenie.
...
\subsection{chage}
Jak pisze autor portalu \textbf{tecmint} \footnote{\url{https://www.tecmint.com/manage-user-password-expiration-and-aging-in-linux/}} polecenie \textbf{chage} jest jednym z ważniejszych w zarządzniu użytkownikami. Często w zadaniach egzaminacyjnych mamy polecenia w rodzaju: ustaw czas życia konta, czas życia hasła.
\subsubsection{przykłady zastosowania chage}
Odczyt aktualnych info o użytkowniku:
begin{verbatim}
chage -l student
man chage
end{verbatim}
zaś dobrze opisuje pozostałe opcje.
\subsubsection{godziny logowania}
W 64-bit wersji useradd jest możliwość dodania rozszerzonych opcji (-x extented attributes) i tak:
begin{verbatim}
useradd normal options -x logon_hours=Wk0800-1600 romek
end{verbatim}
Romek będzie się logował tylko w godzinach pracy.
...
Ufw domyślnie nie zamyka żadnych portów. Na początku trzeba zstosować ogólną zasadę bezpieczeństwa polegającą na zamknięciu dostępu do wszystkiego co ie jest nam konieczne:
Domyślna polityka ufw jest opisana w pliku /etc/default/ufw i może być zmieniona z linii poleceń:\newline
...
bez czego możemy zostać odcięci od konfigurowanego serwera. Po zmianie ustawień sprawdzamy:
\textbf{ufw status}\newline
jeśli ok:
\textbf{ufw reload}
...
\footote{Wartościowy artykuł: \url{https://wiki.archlinux.org/index.php/Disk\_cloning#Cloning\_a\_partition}}.
\susubsection{Clonezilla}
To bardzo wartościowe narzędzie do klonowania przydatne zwłaszcza w mieszanym środowisku, koniecznośći szybkiego ,,postawienia" całej pracowni lub w przerwie pomiędzy egzaminami, kiedy to musimy zapewnić identyczność środowiska na każdym stanowisku. \textbf{Clonezilla} choć dobrze udokumentowana wymaga od administratora pewnej wiedzy i doświadczenia w konfiguracji i użytkowaniu. Istnieje także odmiana \textit{live} do użytku \textit{ad hoc} zarówno pełnego serwera jak i samej \textbf{Clonezilli} tylko do kopiowania dysk--dysk \footnote{Wiele można nauczyć się z tutoriali: 1.:\url{https://www.youtube.com/watch?v=4Sji20-Qp6k} 2.:\url{https://www.youtube.com/watch?v=RkBetLzsWPY} i 3.:\url{https://www.youtube.com/watch?v=nfnINtFYroQ}} oraz artykułu: \url{https://www.howtoforge.com/cloning-linux-systems-with-clonezilla-server-edition-clonezilla-se}. Istotna jest ,,idea" działania tego rozwiązania, którego bazą jest \textbf{DRBL} \textit{Diskless Remote Boot in Linux} \footnote{http://drbl.sourceforge.net/} \footnote{DRBL uses PXE/etherboot, NFS, and NIS to provide services to client machines so that it is not necessary to install GNU/Linux on the client hard drives individually. Once the server is ready to be a DRBL server, the client machines can boot via PXE/etherboot (diskless). "DRBL" does NOT touch the client hard drives, therefore, other operating systems (e.g. MS Windows) installed on the client machines will be unaffected.}. \textbf{DRBL} zapewnia pełne sieciowe środowisko z serwerem dhcp. Najkorzystniej dokonać pełnej instalacji na pojemnym (obrazy dysków) serwerze wyposażonym w dwie karty sieciowe: ,,zewnętrzną" do Internetu i ,,wewnętrzną" dla klientów.
\begin{enumerate}
\item{Przygotowanie}
\begin{enumerate}
\item{Instalacja DRBL}
...
\subsubsection{dmesg}
\subsection{Podsumowanie narzędzi do badania sprzętu}
W artykule ,,16 commands to check hardware information on Linux'' \footnote{\url{https://www.binarytides.com/linux-commands-hardware-info/}}  mamy zebranych kilkanaście narzędzi wraz z opisem. Wydaje się, że powinno to wyczerpać nasze zapotrzebowanie na testy sprzętowe.
\section{backup}
...
\subsubsection{Instalacja virt-managera}
\begin{verbatim}
  502  usermod --append --groups libvirt libvirt-qemu adam
  503  usermod --append --groups libvirt adam
  504  usermod --append --groups libvirt-qemu adam
  505  groups adam
  506  service qemu-guest status
  507  apt-get update
  508  apt-get install qemu-guest agent
  509  apt-get install qemu-guest-agent
  510  service qemu-guest status
  511  service qemu-guest-agent status
  512  history|tail
  513  history|tail -n12 > ~adam/histag && chown adam: histag
\end{verbatim}
\section{Zadania PSO}
\subsection{klasa 3-cia}
\subsubsection{Zadania klient-serwer w maszynie wirtualnej (qemu-kvm w virt-managerze}
\begin{enumerate}
\item{Konfiguracja i uruchomienie serwera dhcp w sieci wydzielonej}

\begin{enumerate}
\item{Bardzo krótko opisz rolę serwera dhcp w sieci lokalnej}
\item{Uruchom ubuntu server i klient w sieci izolowanej. Sprawdź czy serwer dhcp w twojej sieci działa. B. krótko opisz jak to sprawdzisz}
\item{Wyłącz ubuntu serwer. Zainstaluj nowy ze wskazanego obrazu iso. Skonfiguruj serwer dhcp na nowym serwerze. Przetestuj działanie serwera wg. twojej procedury testującej. Napisz b. krótkie sprawozdanie.}


\begin{enumerate}
\item{Informacje wstępne o DNS typu autorytatywnego}
DNS tego typu odpowiada jedynie za własną strefę; nie obsługuje (recursive queries); jest szybki; nie przechowuje zapytań innych serwerów; nie przekazuje zapytań.\newline
Dla potrzeb ćwiczenia skonfigurujemy tylko jeden (!) główny serwer dla naszej strefy. Skonfigurujemy także możliwość łączenia się klientów z serwerem www.

\item{Konfiguracja}
Na maszynie przeznaczonej do roli DNS edytujemy \/etc\/hosts:\newline
begin{verbatim}
sudo nano /etc/hosts
127.0.0.1	localhost
192.168.1.62	dns.p58 dns

sudo nano /etc/hosts
wpisujemy nazwę naszego dns'a
dns
wymuszamy wczytanie zmian w pliku hostname:
sudo hostname -F /etc/hostname
end{verbatim}
 <=mapujemy adres ip z pełną nazwą:

\item{Instalacja programu NSD}
Po instalacji NSD serwis powinien działać jako użytkownik nsd. Niestety instalator nie zakłada takiego użytkownika i aby uniknąć błędów podczas uruchamiania trzeba przed instalacją NSD ręcznie dodać użytkownika nsd: 
\begin{verbatim}
sudo useradd -r nsd
sudo apt-get update
